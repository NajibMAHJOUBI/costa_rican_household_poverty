...
class TrainValidationXGBoost:
    ...
    def run(self):
        ...
        file_classifier.write("index,max_depth,n_estimators\n")

        # 'max_depth': [2, 4, 6], 'n_estimators'
        index = 0
        for params in get_grid_parameters():
            for max_depth, n_estimators in product(params["max_depth"], params["n_estimators"]):
                ...
                file_classifier.write("{0},{1},{2}\n".format(index, max_depth, n_estimators))
