...
class GaussianMixtureTaskTest {
  ...
  @Test def testKMeans(): Unit = {
    val gaussianMixture = new GaussianMixtureTask(featuresColumn, predictionColumn)
    gaussianMixture.defineModel(kClusters)

    val estimator = gaussianMixture.estimator
    ...
    gaussianMixture.fit(dataFrame)
    val model = gaussianMixture.model
    ...
    val transform = gaussianMixture.transform(dataFrame)
