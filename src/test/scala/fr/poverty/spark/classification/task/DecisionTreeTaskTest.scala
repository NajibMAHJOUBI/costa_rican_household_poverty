...
/**
  ...
  *
  * Decision Tree classifier test suite
  *
  */
...
class DecisionTreeTaskTest extends AssertionsForJUnit  {
  ...
  @Test def testDecisionTreeClassifier(): Unit = {
    val decisionTree = new DecisionTreeTask(labelColumn, featureColumn, predictionColumn)
    decisionTree.defineEstimator
    decisionTree.fit(data)
    val transform = decisionTree.transform(data)
